...
REQUIRED_NAMESPACES=("argocd" "metallb-system" "traefik")
...
check_storage_class() {
    ...
    if kubectl get storageclass &> /dev/null && [ "$(kubectl get storageclass --no-headers 2>/dev/null | wc -l)" -gt 0 ]; then
        ...
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            ...
            print_warning "Proceeding without storage class"
            print_warning "Note: Traefik persistence will need to be disabled"
        fi
    fi
}
...
create_summary() {
    ...
    echo "3. Run: cd argocd && ./bootstrap.sh"
    echo "4. Apply app-of-apps: kubectl apply -f argocd/manifests/base/app-of-apps.yaml"
